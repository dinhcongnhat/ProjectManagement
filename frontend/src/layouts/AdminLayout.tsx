...
import {
    LayoutDashboard, CheckSquare, Users, FolderOpen, Columns3, ListTodo,
...
const adminNavItems = [
    ...
    { icon: Users, label: 'Quản lý nhân viên', path: '/admin/users', mobileLabel: 'Nhân viên' },
    { icon: Columns3, label: 'Làm việc nhóm', path: '/admin/kanban', mobileLabel: 'Nhóm' },
    { icon: FolderOpen, label: 'Thư mục', path: '/admin/folders', mobileLabel: 'Thư mục' },
    { icon: ListTodo, label: 'Công việc cá nhân', path: '/admin/my-tasks', mobileLabel: 'Việc tôi' },
];
...
const AdminLayout = () => {
    ...
    useEffect(() => {
        ...
        const fetchCount = async () => {
            try { const res = await api.get('/notifications/unread-count'); setUnreadCount(res.data.unreadCount); } catch { }
        };
        fetchCount();
    ...
    return (
        <LayoutWrapper>
            <SidebarDesktop>
                <SidebarContentInner items={adminNavItems} onNavClick={() => { }} onLogout={handleLogout} showClose={false} />
            </SidebarDesktop>
                ...
                <HeaderBar>
                    ...
                    <div style={{ position: 'relative' }} ref={notifRef}>
                        ...
                        <AnimatePresence>
                            {showNotifications && (
                                <>
                                    <motion.div className="md:hidden fixed inset-0 bg-black/20 z-[80]"
                                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
...
const SidebarContentInner = ({ items, onNavClick, onLogout, showClose, onClose }: SidebarContentInnerProps) => (
    <>
        ...
        <SidebarNavSection style={{ flex: 1, overflowY: 'auto' }}>
            <SidebarNavLabel>Quản lý</SidebarNavLabel>
            {items.map(item => (
                <SidebarNavLinkStyled key={item.path} to={item.path} end={item.path === '/admin'} onClick={onNavClick}>
