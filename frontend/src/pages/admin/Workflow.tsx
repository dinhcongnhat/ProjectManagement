        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-gray-500 mt-2">Quản lý và theo dõi quy trình làm việc</p>
                </div>
                <div className="flex gap-2">
                    <button className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors shadow-lg shadow-gray-200/50">
                        <Settings size={16} />
                        Tùy chỉnh
                    </button>
                ...
                <div className="h-full overflow-x-auto px-4 lg:px-0 pb-4">
                    <div className="flex gap-4 lg:gap-6 h-full min-w-max lg:min-w-0">
                        {columns.map((col, colIndex) => (
                            <div key={col.id} className={`w-72 lg:w-80 flex flex-col ${col.bgColor} rounded-2xl border ${col.borderColor} max-h-full shrink-0 shadow-lg`}>
                                {/* Column Header */}
                                <div className="p-4 flex items-center justify-between border-b border-white/50 bg-white/50 backdrop-blur-sm rounded-t-2xl shrink-0">
                                    ...
                                    <button className="p-1.5 text-gray-400 hover:bg-white/80 rounded-lg transition-colors">
                                ...
                                <div className="p-3 flex-1 overflow-y-auto space-y-3 scrollbar-hide">
                                    {col.items.map((item) => (
                                        <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-lg shadow-gray-200/50 hover:shadow-xl hover:scale-[1.02] transition-all cursor-grab group">
                                    ...
                                    {/* Add Card Button */}
                                    <button className="w-full py-3 flex items-center justify-center gap-2 text-gray-500 hover:text-gray-700 hover:bg-white/80 rounded-xl border-2 border-dashed border-gray-300 hover:border-gray-400 transition-all group">
